...
namespace SchoolManagement.Service
{
    public class ConductService : IConductService
    {
        ...
        public async ValueTask<IEnumerable<ConductDisplayModel>> GetListClassesInSemester(int grade, string semesterId, ConductQueryModel queryModel)
        {
            try
            {
                var classes = await _context.ClassEntities
                    ...
                    .Select(c => new ConductDisplayModel
                    {
                        ClassName = c.ClassName,
                        AcademicYear = c.AcademicYear,
                        TotalStudents = c.ClassDetails.Count,
                        HomeroomTeacherName = c.HomeroomTeacher.FullName
                    })
                    .ToListAsync();

                // Áp dụng logic tìm kiếm nếu có
                if (!string.IsNullOrEmpty(queryModel.SearchValue))
                {
                    classes = classes.Where(c =>
                        c.ClassName.Contains(queryModel.SearchValue, StringComparison.OrdinalIgnoreCase) ||
                        c.AcademicYear.Contains(queryModel.SearchValue, StringComparison.OrdinalIgnoreCase) ||
                        c.HomeroomTeacherName.Contains(queryModel.SearchValue, StringComparison.OrdinalIgnoreCase)
                    ).ToList();
                }
            ...
            {
                _logger.LogError("An error occurred while getting list of classes in semester. Error: {ex}", ex);
                throw;
            }
        }


        public async ValueTask CreateConduct(string studentId, string semesterId)
        {
